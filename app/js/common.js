...
$(function () {
	...
	$(".toggle-class").on("click", function () {
		...
		if (!$(this).hasClass(class_name)) $(this).addClass(class_name);else $(this).removeClass(class_name);
	});
	...
	if (Cookies.get("special") != 1) $("aside .dropdown span").click(function () {
		var $this = $(this);
	...
	if (Cookies.get("special") != 1) $('.history .slider').slick({
		dots: false,
		infinite: true,
		autoplay: false,
		autoplaySpeed: 5000,
	...
	if (Cookies.get("special") != 1) $('#victory-slider .slider').slick({
		dots: false,
		infinite: true,
		autoplay: true,
		autoplaySpeed: 5000,
	...
	if (Cookies.get("special") != 1) $('#credit-company .slider').slick({
		dots: false,
		infinite: true,
		autoplay: true,
		autoplaySpeed: 5000,
	...
	if (Cookies.get("special") != 1) $('#new-incubator.have-slider .bot-block').slick({
		dots: false,
		infinite: true,
		autoplay: true,
		autoplaySpeed: 5000,
	...
	if (Cookies.get("special") != 1) $('.partners').slick({
		dots: false,
		infinite: true,
		autoplay: true,
		autoplaySpeed: 10000,
	...
	$(".counter:not([class='counter not-counter'])").each(function (i, el) {
		if ($(this).html().length > 5) $(this).css({ "font-size": "60px" });
	});
	...
	if ($(".content").height() < $("main aside").height() && $(window).width() > 768 && $(window).height() > 600) $(".content").css({ "min-height": $(window).height() - $("header").height() + "px" });
	...
	if ($(window).width() < 770) $("aside").height($("body").height());
...
$(window).on("load resize", function () {
	$(".partners a").each(function () {
		if ($(this).children("span").html() == "") $(this).css({
			"background-position": "left top",
			"background-size": "100% 100%"
		});
		if (Cookies.get("special") == 1 && $(this).children("span").html() == "") $(this).remove();
	});
});
...
$(window).on("scroll load resize", function () {
	//change_pos();
	if ($(".counter").length) {
		if ($(window).scrollTop() + $(window).height() >= $(".counter").offset().top && !$(".counter").hasClass("countered")) $(".counter:not([class='counter not-counter'])").addClass("countered").countTo();
	}
});
...
$(function () {
	if (Cookies.get("special") != 1) $(".main-slider").slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		speed: 1000,
	...
	if (Cookies.get("special") != 1) $(".faq__block-head").click(function () {
		var $this = $(this);
	...
	if (Cookies.get("special") != 1) $(".support-slider").slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		fade: true,
		speed: 1000,
		...
		makeTable: function makeTable() {
			...
			while (true) {
				...
				if (i == 0) {
					...
					table.itog = this.summ * this.persent / 100 / 12 / (1 - Math.pow(1 + this.persent / 100 / 12, -(!this.payInLastMonths ? this.curSrok : this.curSrok - this.curOtsrok)));
					...
					if (this.summ * this.persent / 100 / 12 / (1 - Math.pow(1 + this.persent / 100 / 12, -(!this.payInLastMonths ? this.curSrok : this.curSrok - this.curOtsrok))) < +this.tableArr[i - 1].ostatok) {
						table.itog = this.summ * this.persent / 100 / 12 / (1 - Math.pow(1 + this.persent / 100 / 12, -(!this.payInLastMonths ? this.curSrok : this.curSrok - this.curOtsrok)));
					} else {
