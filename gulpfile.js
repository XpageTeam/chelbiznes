...
gulp.task('online-special:sass', function() {
	return gulp.src('app/special-online/*.sass')
		.pipe($.if(g_if, sourcemaps.init()))
		.pipe($.sass({
			includePaths: bourbon.includePaths,
			outputStyle: 'expanded'
		...
		.pipe($.if(g_if, sourcemaps.write(".")))
		.pipe($.if(!g_if, postcss([
				prefixer({
					grid: true,
					browsers: ["last 50 versions", "> 1%", "ie 10"]
				})
			])))
		.pipe(server_conn.dest("/www/chelb.xpager.ru/local/onlinetest/css/"));
});
...
gulp.task("base64-post", function(){
...
gulp.task('imagemin', function() {
	return gulp.src('app/img/**/*', {since: gulp.lastRun("imagemin")})
		.pipe($.cache($.imagemin({
			interlaced: true,
			progressive: true,
...
gulp.task('remove', function(callback) {del.sync('app/css/main_data.css'); callback();});

gulp.task("make:css", gulp.series(pre_build, "remove", gulp.parallel("sass", "imagemin"), "prefixer", "base64", "base64-post"));

gulp.task('removedist', function(callback) {del.sync('dist'); callback();});

gulp.task("build:css", function(){
	return gulp.src("app/css/**/*").pipe(gulp.dest('dist/css/'));
});

gulp.task("build:files", function(callback){return gulp.src('app/*.ico').pipe(gulp.dest('dist'))});

gulp.task("build:fonts", function(){return gulp.src('app/fonts/**/*').pipe(gulp.dest('dist/fonts'))});

gulp.task("build:js", function(){return gulp.src('app/js/**/*').pipe(gulp.dest('dist/js'))});

gulp.task("build:images", function(){return gulp.src("app/img/**/*").pipe(gulp.dest("dist/img"))});

gulp.task("build:html", function(){return gulp.src('app/*.html').pipe(gulp.dest('dist'))});
...
gulp.task("deploy:xpager", function() {
...
gulp.task('clearcache', function (callback) { $.cache.clearAll(); callback();});
...
gulp.task("deploy:css", function(){
	return gulp.src("app/css/*.css", {since: gulp.lastRun("sass")})
		.pipe(server_conn.dest(remotePathCss));
});

gulp.task("deploy:js", function(){
	return gulp.src("app/js/*.js", {since: gulp.lastRun("deploy:js")})
		.pipe($.uglify())
...
function watch(){
	...
	gulp.watch('app/special-online/**/*.sass', gulp.series("online-special:sass"));
	gulp.watch('app/js-es6/**/*.js', gulp.series("babel"));
	gulp.watch('app/js/**/*.js', gulp.series("deploy:js"));
	gulp.watch('app/img/*.*', gulp.series("imagemin", "deploy:img"));
}

gulp.task("finish:him", gulp.series(pre_build, "babel", gulp.parallel("make:css", "deploy:js"), "min:css", "deploy:css"));
